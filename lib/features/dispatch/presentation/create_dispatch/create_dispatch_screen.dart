...
// Riverpod provider to fetch campuses once and cache them
final campusListProvider = FutureProvider<List<CampusModel>>((ref) {
  return ref.watch(dispatchRepositoryProvider).getCampuses();
});
...
class _CreateDispatchScreenState extends ConsumerState<CreateDispatchScreen> {
  ...
  CampusModel? _selectedSenderCampus;
  ...
  Future<void> _handleCreateDispatch() async {
    if (_titleController.text.trim().isEmpty) {
      _showSnack('Please enter a dispatch title');
      return;
    }
    if (_selectedSenderCampus == null) {
      _showSnack('Please select a sender campus');
      return;
    }
    if (_selectedDestination == null) {
      _showSnack('Please select a destination campus');
      return;
    }
    ...
    try {
      ...
      final dispatch = DispatchModel(
        ...
        sourceCampusId: _selectedSenderCampus!.id,
        destinationCampusId: _selectedDestination!.id,
      ...
      await ref.read(dispatchRepositoryProvider).createDispatch(dispatch);

      if (mounted) {
        _showSnack('Dispatch created successfully!');
  ...
  Widget build(BuildContext context) {
    final campusAsync = ref.watch(campusListProvider);
          ...
          children: [
            ...
            _buildSectionCard(
              ...
              children: [
                ...
                campusAsync.when(
                  data: (campuses) => _buildCampusDropdownField(
                    value: _selectedSenderCampus,
                    hint: 'Sender Campus',
                    icon: Icons.unarchive_outlined,
                    items: campuses,
                    onChanged: (val) =>
                        setState(() => _selectedSenderCampus = val),
                  ),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              hintStyle: const TextStyle(
                  color: LoopitColors.grey300, height: 1.4),
              isDense: true,
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: hint,
                hintStyle:
                    const TextStyle(color: LoopitColors.grey300),
              ),
            ),
          ),
        ],
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                icon: const Icon(Icons.keyboard_arrow_down,
                    color: LoopitColors.black, size: 20),
                isExpanded: true,
                style: const TextStyle(
                  fontFamily: 'Inter',
                  fontSize: 14,
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<CampusModel>(
                ...
                items: items.map((item) {
                  return DropdownMenuItem(
                    value: item,
                    child: Text(item.campusName ?? item.id),
                  );
                }).toList(),
                onChanged: onChanged,
              ),
